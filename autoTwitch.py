...
from config import username, password, streamerURL, alreadyGot
...
class AutoStream:
    ...
    def run(self):
        ...
        personalAlreadyGot = [False] * len(alreadyGot)
        
        for i, b in enumerate(alreadyGot):
            personalAlreadyGot[i] = b
        
        for i in range(0, len(streamerURL)):
            if not alreadyGot[i] and not personalAlreadyGot[i]:
                links = streamerURL[i]
